...
### 2. 対戦リザルトの取り込み・確認画面（未登録生徒の登録 ＆ 相手名の学習辞書）
...
- **対戦相手名のOCR確認と自動リネーム辞書**:  
  対戦相手の名前はフォントや文字装飾によりOCR認識精度に差が出ることがあります。そのため、最初のうちは右側に並んで表示されるトリミング元画像を確認しながら正しい名前に修正してください。  
  **一度手動で修正して登録を完了すると自動的に「修正辞書」へ学習・登録**され、次回以降は同じ相手の名前が自動で正しい表記に変換されます。

---

### 3. 戦績履歴画面とスマート絞り込み（「匿名」対策・固有武器記録）
...
- **カスタム枠による固有武器ランク記録（青/黄）**:  
  匿名プレイヤーは名前による判別が難しいため、相手の防衛編成（特に D5 / D6 のSpecial生徒やアタッカー）の**固有武器ランク（固有2・固有3など）で絞り込み・識別**を行うのが効果的です。  
  戦績詳細画面のメモ欄を使って右端に**青（攻撃側）や黄色（防衛側）の数字（例: `2` `2`、`2` `3`）**を記録しておくと、テーブル一覧の右端にバッジ表示され、同一相手の特定や対策の検討に非常に便利です。
